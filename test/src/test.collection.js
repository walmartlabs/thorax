$(function() {
  ...
  test("filter what items are rendered in a collection", function() {
    ...
    equal(view.$('li').length, 2, 'add item that should not be included');
    equal(view.$('li')[1].innerHTML, 'b', 'add item that should not be included');
    c.set({key: 'b'});
    equal(view.$('li').length, 3, 'set item not included to be included');
    equal(view.$('li')[1].innerHTML, 'b', 'set item not included to be included');
    equal(view.$('li')[2].innerHTML, 'b', 'set item not included to be included');
    c.set({key: 'c'});
    equal(view.$('li').length, 2, 'set item that is included to not be included');
    equal(view.$('li')[1].innerHTML, 'b', 'set item that is included to not be included');
    a.set({key: 'x'});
    equal(view.$('li').length, 1, 'set first included item to not be included');
    equal(view.$('li')[0].innerHTML, 'b', 'set first included item to not be included');
    a.set({key: 'a'});
    equal(view.$('li').length, 2);
    equal(view.$('li')[0].innerHTML, 'a', 'set first item not included to be included');
    equal(view.$('li')[1].innerHTML, 'b', 'set first item not included to be included');
    a.set({key: 'a'});
    equal(view.$('li')[0].innerHTML, 'a', 'items maintain order when updated when filter is present');
    equal(view.$('li')[1].innerHTML, 'b', 'items maintain order when updated when filter is present');
    b.set({key: 'b'});
    equal(view.$('li')[0].innerHTML, 'a', 'items maintain order when updated when filter is present');
    equal(view.$('li')[1].innerHTML, 'b', 'items maintain order when updated when filter is present');
  });
